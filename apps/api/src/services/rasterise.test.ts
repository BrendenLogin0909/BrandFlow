...
describe('rasterisePage', () => {
  ...
  it('reports the brand fonts it could not load instead of pretending they rendered', async () => {
    const out = await rasterisePage(PAGE_SVG, { resolveRemoteImages: false });
    // no font dirs are configured in tests, so Poppins cannot be resolved
    expect(out.fontFallbacks).toContain('Poppins');
  });
