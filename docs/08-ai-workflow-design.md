...
**Step 7b — freeform compose (creative mode, implemented in DesignGenerationService).** When a generation runs with `mode: 'freeform'`, the AI composes the full layout itself — element placement, sizes, layering, groups, motifs — emitting pages of schema elements directly. Freedom is bounded by hard constraints, not templates: token-only colours, brand fonts, licensed icon refs, approved assets, schema element types, and the full validation engine with one violation-guided repair round. Near-miss results (≤2 residual errors) are kept and flagged for human fix; unrepairable output falls back to recipe mode, so creativity can never ship a broken design. Contrast enforcement follows the brand's `contrastMode` setting ('enforce' default; 'warn' downgrades contrast to reported warnings).
