...
- **TO RUN:** apply migration `apps/api/prisma/migrations/20260707140000_revision_reason_ai_patch` (`ALTER TYPE "RevisionReason" ADD VALUE 'AI_PATCH'`) via `prisma migrate deploy`, then stop API → `prisma generate` → restart (DLL-lock gotcha). Client already regenerated locally.
- **Next:** P3-E "Edit with AI" Studio UI (Agent 8) consumes this endpoint; P3-G draft-stage visual direction (Agent 9).
